...
static vector< vector<tinyobj::material_t> > materialList;
...
bool OBJ::load(const std::string& fileName)
{
    ...
    materialList.push_back(materials);
    materialsId = materialList.size() - 1;

    // setting
    normalizedScale = 0.5f / maxExtent;
    normalizedPosition[0] = -0.5 * (bmax[0] + bmin[0]);
    normalizedPosition[1] = -0.5 * (bmax[1] + bmin[1]);
    normalizedPosition[2] = -0.5 * (bmax[2] + bmin[2]);
    loaded = true;
...
void OBJ::free()
{
    if(!available())
        return;
    for(map<string, unsigned int>::iterator it = textures.begin(); it != textures.end(); it++)
    {
    ...
    for (size_t i = 0; i < drawObjects.size(); i++) 
    {
        ...
        glDeleteBuffers(1, &o.vb_id);
    }
    drawObjects.clear();

    if(materialsId >= 0)
    {
        materialList.erase(materialList.begin() + materialsId);
        materialsId = -1;
    }
...
string OBJ::GetBaseDir(const string& filepath) 
{
...
bool OBJ::FileExists(const string& abs_filename) 
{
    bool ret;
